-- ==============================================
-- MESSAGES + GROUP ROLES SETUP (Car-Pes) - FIXED
...
-- =====================================================
-- REBUILD DIRECT MESSAGES
-- =====================================================
DROP TABLE IF EXISTS direct_message_likes CASCADE;
DROP TABLE IF EXISTS direct_messages CASCADE;
DROP TABLE IF EXISTS direct_chat_hidden CASCADE;
...
CREATE TABLE direct_chat_hidden (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_direct_chat_hidden_user_partner ON direct_chat_hidden(user_id, partner_id);
CREATE INDEX idx_blocked_users_blocker_blocked ON blocked_users(blocker_id, blocked_id);
CREATE INDEX idx_direct_message_likes_message ON direct_message_likes(message_id);
...
-- =====================================================
-- GROUP TABLES - SETUP ROLE COLUMN
-- =====================================================
ALTER TABLE IF EXISTS chat_group_members ADD COLUMN IF NOT EXISTS role TEXT DEFAULT 'member';

UPDATE chat_group_members SET role = 'member' WHERE role IS NULL;
...
-- =====================================================
-- DIRECT MESSAGE POLICIES - SELECT, INSERT, UPDATE, DELETE (separate)
-- =====================================================
...
-- =====================================================
-- MESSAGES POLICIES (Group messages in 1-1 context)
-- =====================================================
...
-- ✅ Setup completado sin errores de sintaxis.
